...
function attachEventListeners() {
  ...
  // Auto-update on input change (excluding gain/loss and percentage to prevent interference)
  const nonAutoUpdateInputs = document.querySelectorAll(
    "input:not(#gainLoss):not(#percentage), select",
  );
  nonAutoUpdateInputs.forEach((input) => {
...
function updatePreview() {
  ...
  const buyingPower =
    parseFloat(document.getElementById("buyingPower").value) || 0;
  ...
  performanceEl.className = isPositive
    ? "performance positive"
    : "performance negative";

  // Map selected period to time label
  const timeLabelMap = {
    LIVE: "Past hour",
    "1D": "Today",
    "1W": "Past week",
  ...
  const timeLabel = timeLabelMap[selectedPeriod] || selectedPeriod;
  ...
  // Update buying power
  document.getElementById("buyingPowerAmount").textContent =
  ...
  const activePeriodBtn = document.querySelector(
    `.time-btn[data-period="${selectedPeriod}"]`,
  );
  if (activePeriodBtn) {
  ...
  // Update settings button color based on positive/negative
  const settingsBtn = document.querySelector(".time-btn-settings");
  if (settingsBtn) {
    settingsBtn.classList.remove("positive", "negative");
    settingsBtn.classList.add(isPositive ? "positive" : "negative");
  }
...
function drawChart() {
  ...
  // Chart dimensions - adjust padding for proper alignment with 440px width
  const paddingTop = 10;
  ...
  // Set colors based on positive/negative and light/dark mode
  const lineColor = isPositive ? "#00c805" : "#ff6400";

  // Draw smooth line
  ctx.beginPath();
  let x = paddingLeft;
  let y = paddingTop + (1 - (chartData[0] - minValue) / valueRange) * chartHeight;
  ctx.moveTo(x, y);
